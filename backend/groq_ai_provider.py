...
def try_groq_ai(
    ...
    try:
        client = OpenAI(
            ...
            # =====================================================
            # منع إعادة المحاولة التلقائية
            # =====================================================
            #
            # المشكلة التي عالجها هذا السطر:
            #
            #   عند تجاوز حد Groq المجاني (رمز 429)، كانت مكتبة OpenAI
            #   تنتظر تلقائيًا عبر time.sleep داخلي ثم تعيد المحاولة.
            #   هذا الانتظار يجمّد عامل Gunicorn، فيَعتبره الخادم معلّقًا
            #   ويقتله بإشارة SIGKILL — فيعود الطلب بخطأ 500.
            #
            #   وبما أننا نملك مسار احتياطيًا (المحلل المحلي)، لا حاجة
            #   لانتظار إعادة المحاولة: نفشل فورًا وننتقل للمحلي.
            #
            # max_retries=0 يلغي الانتظار الداخلي كليًا.
            max_retries=0,
        )
